                  ...
                  <div class="d-flex flex-column flex-md-row align-items-center justify-content-between">
                      <div class="statistics-item">
                       ...
                       All orders
                        ...
                        $query = $db->query('SELECT * FROM orders WHERE ClientId='.$id.'');
                         echo $query->getNumRows();
                        ?></h2>
                     <a href="<?=base_url('orders_all')?>" > <label class="badge badge-outline-success badge-pill">All Orders</label> </a>
                      </div>
                      <div class="statistics-item">
                        <p>
                        ...
                        <h2><?php
                          $query = $db->query('SELECT * FROM orders WHERE ClientId='.$id.' AND OrderStatus=1');
                         echo $query->getNumRows();
                        ?></h2>
                      <a href="<?=base_url('orders_inprogress')?>" >  <label class="badge badge-outline-danger badge-pill">In Progress</label></a>
                      </div>
                      <div class="statistics-item">
                        <p>
                        ...
                        <h2><?php
                         $query = $db->query('SELECT * FROM orders WHERE ClientId='.$id.' AND OrderStatus=4');
                         echo $query->getNumRows();
                        ?></h2>
                     <a href="<?=base_url('orders_completed')?>"> <label class="badge badge-outline-success badge-pill">Completed</label> </a>
                      </div>
                      <div class="statistics-item">
                        <p>
                        ...
                        <h2><?php
                        $query = $db->query('SELECT * FROM orders WHERE ClientId='.$id.' AND OrderStatus=3');
                         echo $query->getNumRows();
                        ?></h2>
                     <a href="<?=base_url('orders_inreview')?>" >   <label class="badge badge-outline-success badge-pill">View</label></a>
                      </div>
                      <div class="statistics-item">
                        <p>
                        ...
                        <h2><?php
                        $query = $db->query('SELECT * FROM orders WHERE ClientId='.$id.' AND OrderStatus=2');
                         echo $query->getNumRows();
                        ?></h2>
                      <a href="<?=base_url('orders_rejected')?>" > <label class="badge badge-outline-success badge-pill">Rejected</label></a>
                      </div>
...
<?php
$query =$db->query("SELECT * FROM orders WHERE ClientId=$id ORDER BY ID DESC LIMIT 3");
?>
